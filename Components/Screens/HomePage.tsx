...
function HomePage() {
    ...
    const locationFetecher = async ()=>{
        ...
        if(context.location?.length>0){
          let response = await getLocationDetails(context.location.latitude,context.location.longitude);
        ...
        then(  (response:any)=>{
            ...
            postLocation(response.data.results[0].formatted,moment().format("HH:mm:ss"))
        })
    ...
    const removeLocationHandler = (id:number)=>{
        context.setCurrentLocation();
        context.setLocationDetails(context.locationDetails?.filter((data:any,index:number)=>{
            return(
                index!==id
            )
        }))
        context.setTime(context.time?.filter((data:any,index:number)=>{
             return(
                index!==id
             )
        }))
        context.setDate(context.date?.filter((data:any,index:number)=>{
             return(
                index!==id
             )
        }))
    ...
    return (
       <View style={styles.container} >
        ...
        <View >
            <Text testID='list-current-label'style={styles.current}>Current Location</Text>
        </View>
        ...
        <View >
            <Text testID="list-current-name" style={styles.location_text} numberOfLines={1}>{context.currentLocation?.length>0 ?context.currentLocation[0]?.formatted : "Wait location is loading.."}</Text>
        </View>
        <View>
            <Text style={styles.time} testID="list-current-time">{moment().format("MM/DD/YYYY")}, {moment().format("HH:mm:ss")}</Text>
        </View>
        </View>
        </View>
        <View>
            ...
            renderItem={(itemdata)=>{
                return(
                    <View style={styles.location_data}>
                        ...
                        <Text testID = {`List-previous-name-${itemdata.index}`} style={styles.location_name}  numberOfLines={1}>{itemdata.item.formatted}</Text>
                        <Text testID = {`List-previous-time-${itemdata.index}`} style={styles.time}>{context.date[itemdata.index]},{context.time[itemdata.index]}</Text>
                        </View>
                        <View>
                        <Button 
